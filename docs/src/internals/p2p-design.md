...
Overlaybd remote layer opens try the UUID facade first when the committed layer has a UUID, then `/p2p-digest/{sha256:hex}` using the committed digest. The digest endpoint reuses `overlaybd-layer/v1/sha256:<digest>` and validates layer metadata before serving bounded byte ranges. Both endpoints only read P2P artifacts; an open failure returns control to Overlaybd's existing OSS or registry source. This lets memory layers without UUIDs use P2P without moving OSS credentials or object-store access into the HTTP facade. Missing explicit local files retain the existing local-file behavior.
