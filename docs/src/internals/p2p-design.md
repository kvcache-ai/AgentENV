...
The OSS repository returns the committed record together with the prepared local layer uploads. The manager publishes their existing digest/size descriptors without computing SHA256 again, and keeps compressed or dense-exported temporary files alive until the post-commit P2P copies finish. These local paths never enter the durable record. UUID publication from the runtime image configs remains available; duplicate digest keys are published once.

Digest-keyed publication from runtime image configs is still guarded against the committed record: a local layer is only advertised under `overlaybd-layer/v1/sha256:<digest>` when that digest is one the record references for the same subject (memory, rootfs, or the matching attached drive). When `[snapshot.publish_compression]` recontainerizes a raw local layer as ZFile, the prepared upload supplies the compressed bytes and their descriptor; the original raw digest is skipped. ZFile layers carry no LSMT uuid (`uuid = None` in the committed record). Publishing these digest keys does not add a digest-only OSS read route; that consumer path is a separate change.
