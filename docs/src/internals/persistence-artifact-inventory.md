...
| Firecracker serial logs      | `firecracker.serial_dir/{sandbox_id}/*`, or warm pool work dir logs | Firecracker stdout/stderr                                 | Debugging                                                    | Opened on spawn and appended. Empty managed logs are removed after exit. Background cleanup collects directories marked inactive after confirmed process exit and expires their nonempty logs after `firecracker.serial_log_retention_secs` (default seven days; zero disables expiry). Unmarked legacy and abnormal-drop logs are preserved. | No, but disposable.                                          |
| Firecracker logger output    | `firecracker.log` in the same per-sandbox log directory as the serial logs | Firecracker internal logger (`PUT /logger`)               | Debugging                                                    | Only created when `firecracker.log_level` is set to a non-empty level. Uses the same empty-log cleanup and retention as serial output. | No, but disposable.                                          |
