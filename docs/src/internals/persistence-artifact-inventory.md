...
| Firecracker manifest | POSIX: `<snapshot_store>/repository/snapshots/{id}/firecracker-manifest.json`; OSS: `artifacts/{id}/firecracker-manifest.json` | Firecracker snapshot shape, virtual sizes, attached-drive metadata, and the snapshot-time virtio-mem policy/requested size; path fields are hydrated at runtime. Legacy manifests without the virtio-mem field deserialize with memory hotplug disabled | Runtime manifest template for launching committed snapshots | Persisted during publish. Removed with per-snapshot artifacts. | Partially, but kept as durable artifact. |
