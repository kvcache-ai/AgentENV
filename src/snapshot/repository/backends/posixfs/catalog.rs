...
use crate::snapshot::repository::{volume_catalog_shard, SnapshotListFilter, VolumeRecordPage};
use crate::snapshot::{
...
impl PosixFsCatalogStore {
    ...
    fn ensure_layout(&self) -> RepositoryResult<()> {
        ...
        let volume_names_dir = PosixFsSnapshotArtifactLayout::volume_names_dir(&self.root);
        let volume_owners_dir = PosixFsSnapshotArtifactLayout::volume_owners_dir(&self.root);
    ...
    pub(crate) fn get_volume(&self, reference: &str) -> RepositoryResult<Option<VolumeRecord>> {
        ...
        let name_path = PosixFsSnapshotArtifactLayout::volume_name_path(&self.root, reference);
        if !name_path.exists() {
            return Ok(None);
        }
        let volume_id: String = self.read_json(&name_path)?;
        self.ensure_volume_id(&volume_id)?;
        self.load_volume_by_id_unlocked(&volume_id)
    }
    ...
    pub(crate) fn list_volumes_page(
        ...
        let start_shard = after_volume_id
            .map(volume_catalog_shard)
            .unwrap_or_else(|| "00".to_string());
        let start_shard = u8::from_str_radix(&start_shard, 16)
            .map_err(|error| RepositoryError::backend("parse volume catalog shard", error))?;
        let mut selected = Vec::with_capacity(limit.saturating_add(1));

        'shards: for shard_index in start_shard..=u8::MAX {
            let shard = format!("{shard_index:02x}");
            let directory = PosixFsSnapshotArtifactLayout::volume_shard_dir(&self.root, &shard);
            let entries = match fs::read_dir(&directory) {
                Ok(entries) => entries,
                Err(error) if error.kind() == std::io::ErrorKind::NotFound => continue,
                Err(error) => {
                    return Err(RepositoryError::backend(
                        format!("read volume shard '{}'", directory.display()),
                        error,
                    ))
                }
            };
            ...
            for entry in entries {
                let entry = entry.map_err(|error| {
                    RepositoryError::backend("read volume catalog entry", error)
                })?;
                if !entry
                    .file_type()
                    .map_err(|error| {
                        RepositoryError::backend("inspect volume catalog entry", error)
                    })?
                    .is_file()
                ...
                if volume_catalog_shard(volume_id) != shard {
                    return Err(RepositoryError::InvalidRequest {
                        reason: format!(
                            "volume record '{}' is stored in the wrong shard '{shard}'",
                            entry.path().display()
                        ),
                    });
                }
            ...
            for volume_id in volume_ids {
                if shard_index == start_shard
                    && after_volume_id.is_some_and(|after| volume_id.as_str() <= after)
                {
                    continue;
                }
                selected.push(volume_id);
        ...
        for volume_id in selected {
            let record = self
                ...
                .ok_or_else(|| RepositoryError::VolumeNotFound {
    ...
    pub(crate) fn create_volume(&self, record: &VolumeRecord) -> RepositoryResult<()> {
        ...
        let record_path = PosixFsSnapshotArtifactLayout::volume_record_path(&self.root, &record.id);
        if record_path.exists() {
            return Err(RepositoryError::InvalidRequest {
                reason: format!("volume '{}' already exists", record.id),
            });
        }
        let name_path = PosixFsSnapshotArtifactLayout::volume_name_path(&self.root, &record.name);
        if name_path.exists() {
        ...
        self.write_json(&record_path, &durable_record)?;
        if let Err(error) = self.write_json(&name_path, &record.id) {
            let _ = self.remove_file_if_exists(&record_path);
            return Err(error);
        }
        Ok(())
    }
    ...
    pub(crate) fn put_volume(&self, record: &VolumeRecord) -> RepositoryResult<()> {
        ...
        if existing.revision > record.revision {
            return Err(RepositoryError::InvalidRequest {
                reason: format!(
                    "volume '{}' revision {} supersedes attempted revision {}",
    ...
    pub(crate) fn delete_volume(&self, volume_id: &str) -> RepositoryResult<()> {
        ...
        if let Some(owner) = record.read_only_mounts.first() {
            return Err(RepositoryError::InvalidRequest {
                reason: format!("volume '{volume_id}' is mounted read-only by sandbox '{owner}'"),
            });
        }
        ...
        let name_path = PosixFsSnapshotArtifactLayout::volume_name_path(&self.root, &record.name);
        if name_path.exists() && self.read_json::<String>(&name_path)? == volume_id {
            self.remove_file_if_exists(&name_path)?;
        }
        Ok(())
    }
    ...
    pub(crate) fn reserve_volume(
        ...
        if let Err(error) = self.write_volume_owner_entry(owner, volume_id) {
            record.reserved_by_sandbox_id = None;
            let _ = self.write_json(&path, &record);
            return Err(error);
        }
        Ok(None)
    }
    ...
    pub(crate) fn list_volumes_by_owner(&self, owner: &str) -> RepositoryResult<Vec<VolumeRecord>> {
        self.ensure_volume_component(owner, "owner")?;
        self.load_volumes_by_owner_unlocked(owner)
    }
    ...
    pub(crate) fn replace_volume_owner(
        ...
        for mut record in records {
            ...
            if let Err(error) = self.write_json(
                &PosixFsSnapshotArtifactLayout::volume_record_path(&self.root, &record.id),
                &record,
            ) {
                if let Some(to) = to {
                    let _ = self.remove_file_if_exists(
                        &PosixFsSnapshotArtifactLayout::volume_owner_path(
                            &self.root, to, &record.id,
                        ),
                    );
                }
                return Err(error);
            }
            self.remove_file_if_exists(&PosixFsSnapshotArtifactLayout::volume_owner_path(
                &self.root, from, &record.id,
            ))?;
        }
        Ok(())
    }
    ...
    fn load_volumes_by_owner_unlocked(&self, owner: &str) -> RepositoryResult<Vec<VolumeRecord>> {
        let directory = PosixFsSnapshotArtifactLayout::volume_owner_dir(&self.root, owner);
        let entries = match fs::read_dir(&directory) {
            Ok(entries) => entries,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(Vec::new()),
            Err(error) => {
                return Err(RepositoryError::backend(
                    format!("read volume owner index '{}'", directory.display()),
                    error,
                ))
            }
        };
        ...
        for entry in entries {
            let entry = entry
                .map_err(|error| RepositoryError::backend("read volume owner entry", error))?;
            if !entry
                .file_type()
                .map_err(|error| RepositoryError::backend("inspect volume owner entry", error))?
                .is_file()
                || entry.path().extension().and_then(|value| value.to_str()) != Some("json")
            {
                continue;
    ...
    fn write_volume_owner_entry(&self, owner: &str, volume_id: &str) -> RepositoryResult<()> {
        self.write_json(
            &PosixFsSnapshotArtifactLayout::volume_owner_path(&self.root, owner, volume_id),
            &volume_id,
        )
    }
...
mod tests {
    ...
    use crate::snapshot::repository::volume_catalog_shard;
    ...
    fn volume_catalog_persists_and_serializes_reservations_atomically() {
        ...
        store
            ...
            .expect("create volume should work");
        assert_eq!(
            store.list_volumes_page(None, 10).unwrap().records,
            vec![record.clone()]
        );
        ...
        let mut stale_catalog_update = store.get_volume("vol_test").unwrap().unwrap();
        store
            .replace_volume_owner("sandbox-b", None)
            .expect("release should work");
        stale_catalog_update.revision += 1;
        store
            .put_volume(&stale_catalog_update)
            .expect("catalog update should preserve the concurrent release");
        assert!(store
            .get_volume("data")
            .unwrap()
            .unwrap()
    ...
    #[test]
    fn volume_catalog_uses_keyed_indexes_and_bounded_pages() {
        let tempdir = TempDir::new().expect("tempdir should exist");
        let store = PosixFsCatalogStore::new(tempdir.path().to_path_buf());
        ...
        assert_eq!(
            store.get_volume("data-000123").unwrap().unwrap().id,
            "vol_000123"
        );
        assert_eq!(
            store.get_volume("vol_000123").unwrap().unwrap().name,
            "data-000123"
        );
        let conflict = VolumeRecord {
            id: "vol_conflict".to_string(),
            name: "data-000123".to_string(),
            mode: VolumeMode::Exclusive,
            source: "empty".to_string(),
            parent_volume_id: None,
            revision: 0,
        ...
        assert!(matches!(
            store.create_volume(&conflict),
            Err(RepositoryError::VolumeNameConflict { .. })
        ));

        expected_ids.sort_by(|left, right| {
            volume_catalog_shard(left)
                .cmp(&volume_catalog_shard(right))
                .then_with(|| left.cmp(right))
        });
        let mut actual_ids = Vec::new();
        let mut after = None;
        loop {
            let page = store.list_volumes_page(after.as_deref(), 13).unwrap();
            assert!(page.records.len() <= 13);
        ...
        for volume_id in expected_ids.iter().take(4) {
            store.reserve_volume(volume_id, "sandbox-owner").unwrap();
        }
        assert_eq!(
            store.list_volumes_by_owner("sandbox-owner").unwrap().len(),
            4
        );
        store.replace_volume_owner("sandbox-owner", None).unwrap();
        assert!(store
            .list_volumes_by_owner("sandbox-owner")
            .unwrap()
            .is_empty());
    }
}
