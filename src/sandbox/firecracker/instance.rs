...
///
/// Values are trusted backend observations: the i32-to-u32 conversion in
/// `TryFrom` rejects missing and negative fields, and
/// [`validate_memory_hotplug_status`] enforces the geometry and size
/// invariants before a status reaches accounting, the API, or a snapshot.
/// The fields stay public so callers can read and pattern-match them; they
/// are not a user-constructible configuration surface.
...
///
/// Produced only by [`FirecrackerInstance`] operations from validated
/// statuses, never assembled by callers: `elapsed_ms` is the measured
/// convergence time and every size is the device-reported value at
/// convergence.
#[derive(Clone, Debug, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
...
impl FirecrackerInstance {
    ...
    pub async fn patch_memory_hotplug(&self, requested_size_mib: u32) -> Result<()> {
        ...
        self.client
            ...
            .await
            .with_context(|| format!("Failed to request virtio-mem size {requested_size_mib} MiB"))
    }
